...
export const fetchTasks = createAsyncThunk(
  ...
  async (params?: { title?: string; completed?: boolean | null }) => {
    ...
    if (params?.completed !== undefined && params.completed !== null) query.append('completed', String(params.completed));
...
export const addTask = createAsyncThunk('tasks/addTask', async (newTask: Omit<Task, '_id'>) => {
...
export const updateTask = createAsyncThunk('tasks/updateTask', async (updatedTask: Task) => {
  const response = await axios.put(baseUrl+`/tasks/${updatedTask._id}`, updatedTask);
  return response.data;
});
